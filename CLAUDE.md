...
cp .env.example .env
# Set OPENAI_API_KEY=<your key>  (recommended)
# Or set TOGETHER_API_KEY / GROQ_API_KEY and change LLM_PROVIDER accordingly
...
| --------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **OpenAI**      | [https://platform.openai.com/api-keys](https://platform.openai.com/api-keys) | **Recommended — best Arabic quality.** Set `OPENAI_API_KEY=`. |
| **Together.ai** | [https://api.together.xyz](https://api.together.xyz) → Sign up → API Keys    | Free tier: ~60 req/min. Set `TOGETHER_API_KEY=`.              |
| **Groq**        | [https://console.groq.com](https://console.groq.com) → API Keys              | Free tier, fast. Set `GROQ_API_KEY=`.                         |
| **stub**        | None needed                                                                  | Offline testing — canned responses, not real.                 |


Set `LLM_PROVIDER=openai` (or `together` / `groq` / `stub`) in `.env`.
The tier chain auto-selects: OpenAI gpt-4o-mini → Qwen2.5-7B (Together) → Mistral-7B (Together).
...
User query
    │
    ▼
Worker 2 — Agent 1: Query Understanding (Stages 0.5–3)   [Persona: Fatat Al-Arab]
  intent_router → bilingual_analyzer → hyde → bilingual_expand → self_query
    │   (QueryContext)
    ▼
Worker 3 — Agent 2: Retrieval & Synthesis (Stages 4–10)  [Persona: Fatat Al-Arab]
...
Worker 1 — Al-Nassikh: Deterministic ETL (offline / on ingest)  [Persona: Al-Nassikh]
  PAGE-XML → cross_link → registry_join → embed → Qdrant
```

The orchestrator (`src/fatat_al_arab/orchestrator.py`) is the single public entry point.

### Agent Personas (canonical source: `src/fatat_al_arab/personas.py`)


| Persona           | Arabic                           | Role                                                                                                            | Workers       | Key constraint                                              |
| ----------------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------- | ----------------------------------------------------------- |
| **Al-Nassikh**    | الناسخ — The Scribe              | Archival metadata agent; answers counting, dating, and provenance queries from the registry                     | Worker 1      | Never speculates; always cites the registry field           |
| **Fatat Al-Arab** | فتاة العرب — The Arabian Scholar | Bilingual Khaleeji Nabati poetry expert; thematic search, grounded synthesis, CRAG grading, Self-RAG reflection | Workers 2 + 3 | Never fabricates a verse, a poet, or a manuscript reference |


**Al-Nassikh** is activated by the `intent_router` when a query is classified as counting / provenance / dating (`registry_lookup` intent). Her answer is returned in < 2 ms with a 🗂️ badge — no LLM generation involved.

**Fatat Al-Arab** handles all other queries across both agentic graphs. Her persona header is prepended to every LLM call in `self_query`, `crag_grader`, `synthesise`, and `reflect`, ensuring consistent bilingual register and citation discipline throughout the pipeline.

---

## Directory Structure — current state

Legend: ✅ Complete · 🗑️ Legacy (ignore)
...
│       │
│       ├── llm.py                         ✅ Hosted-API adapter (OpenAI / Together / Groq / stub)
│       │                                     chat(prompt, system, json_schema, max_tokens)
│       │                                     3-tier chain: gpt-4o-mini → Qwen2.5-7B → Mistral-7B
│       │                                     Exponential backoff + permanent tier escalation
...
│       │   └── nodes/
│       │       ├── intent_router.py          Stage 0.5a — first node of every turn (M2c)
│       │       │                               Check 0: out-of-scope (sacred texts, non-Nabati
│       │       │                                 poets, non-Arabic epics) → hard refusal
│       │       │                               Check 1: unsupported-dimension (wasm, marginalia,
│       │       │                                 secondary scribe, ink-bleed, stamp coords,
│       │       │                                 handwriting style) → educational answer
│       │       │                               Check 1b: image + provenance cue → image_provenance
│       │       │                               Check 2: capabilities (zero-LLM fast-path)
│       │       │                               Check 3: pipeline-debug (zero-LLM fast-path)
│       │       │                               Check 4: counting / age / provenance →
│       │       │                                 registry_lookup (10+ intents, EN + AR + Khaleeji,
│       │       │                                 genre-slot detection for "how many <genre> poems")
│       │       │                               🆕 Check 4b: poet-name veto — known poet in query
│       │       │                                 → rag_pipeline; UAE leader cluster matched →
│       │       │                                 Qdrant verification gate: counts index chunks for
│       │       │                                 ALL cluster variants; if < 3 sets
│       │       │                                 force_general_knowledge=True (no-corpus fallback)
│       │       │                               🆕 Check 4c: content-query veto — retrieval-intent
│       │       │                                 signals (show/find/give me … poems) → rag_pipeline,
│       │       │                                 skipping prototype router to prevent misrouting
│       │       │                               Check 5: Tier 2 prototype router (AraBERT / TF-IDF)
│       │       │                                 Falls through to semantic_router (Stage 0.5b)
...
│       ├── anchor_registry_full.json      ✅ Phase 1-4 manuscript anchors combined
│       ├── anchor_registry_full_enriched.json ✅ Phase 1-4 anchors + genre/emotions ← CANONICAL SOURCE
...
│   ├── test_prototype_router.py              Prototype / Tier-2 embedding router
│   ├── test_edge_cases.py                    Adversarial + robustness edge cases
...
1. **LLM inference = hosted API only.** 3-tier chain: **OpenAI gpt-4o-mini** (primary, best Arabic) → Qwen2.5-7B via Together.ai (free-tier fallback) → Mistral-7B via Together.ai (last resort). Every LLM call routes through `src/fatat_al_arab/llm.py`. No CUDA / Ollama / vLLM.
...
5. **Silver-baseline genre tagging (🔸 badge required).** Genre/emotion tags come from `genre_heuristic.py` (heuristic_v1). Without a human-labelled gold set, accuracy cannot be claimed — only coverage (82.9% on the manuscript anchors) and distribution can be reported. The 🔸 badge must appear on all heuristic-tagged results in the UI.
...
| **Combined**             | **25 manuscripts**     | **76**   | **2,175**         | ✅ All phases in Qdrant    |
...
> The canonical manuscript registry is `**anchor_registry_full_enriched.json`** (Phase 1–4, 82.9% genre coverage).
> The unified registry `data/unified_registry.json` has **4,031 entries** across all three source types.
> The vector index has **8,415 chunks** across all sources.
